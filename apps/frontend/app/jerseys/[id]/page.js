import { getJerseyById, jerseys } from "../../../data/jerseys";
...
export async function generateMetadata({ params }) {
  ...
  const jersey = getJerseyById(resolvedParams.id);
  if (!jersey) return { title: "Not Found" };
  
  return {
    title: `${jersey.clubName} ${jersey.season} - ${jersey.kitType} | EuroKits`,
    description: jersey.description,
  };
}

export default async function JerseyDetailPage({ params }) {
  // In Next.js App Router (especially recent versions), params should be awaited
  const resolvedParams = await params;
  const jersey = getJerseyById(resolvedParams.id);
  ...
  // Cari jersey yang berkaitan (sama liga, kecuali jersey yang sedang dilihat)
